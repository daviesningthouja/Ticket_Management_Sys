...
const ManageEvents = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const data = role === 'Admin' ? await getAllEvents() : await getOrgEvents();
        setEvents(data);
      } catch (err) {
          ...
          {filteredEvents.map((event, idx) => (
            ...
            >
              <Card
                ...
                img={event.imageUrl}
                alt={event.title}
                value={<div className="text-sm text-gray-700 line-clamp-3">{event.description}</div>}
                description={`${event.location} • ${new Date(event.eventDate).toLocaleString('en-IN', {
